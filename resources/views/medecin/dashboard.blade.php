...
<style>
    ...
    .active-filter { border: 2px solid var(--primary-color) !important; }

    /* Tableau */
    .table-card { border-radius: 15px; border: none; box-shadow: 0 0.15rem 1.75rem 0 rgba(58, 59, 69, 0.1); }
    .table thead th { background: #f8f9fc; border-bottom: none; color: #4e73df; text-transform: uppercase; font-size: 0.8rem; }
</style>
                ...
                <div class="card stat-card bg-danger text-white h-100 {{ request('filter') == 'waiting' ? 'active-filter' : '' }}">
                    <div class="card-body d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h2 class="fw-bold mb-0">{{ $rdvAujourdhui }}</h2>
                        </div>
                ...
                <div class="card stat-card bg-warning text-white h-100 {{ !request('filter') ? 'active-filter' : '' }}">
                    <div class="card-body d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h2 class="fw-bold mb-0">{{ $fileAttente->count() }}</h2>
                        </div>
                ...
                <div class="card stat-card bg-primary text-white h-100">
                    <div class="card-body d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h2 class="fw-bold mb-0">{{ $totalPatientsSuivis }}</h2>
                        </div>
            ...
            <div class="table-responsive">
                <table class="table align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="px-4 fw-bold">{{ \Carbon\Carbon::parse($rdv->appointment_date)->format('H:i') }}</td>
                            <td>
                            ...
                            <td>
                                @if($rdv->status == 'termine')
                                    <span class="badge bg-success-subtle text-success px-3 rounded-pill">Terminé</span>
                                @else
                                    <span class="badge bg-warning-subtle text-warning px-3 rounded-pill">En attente</span>
                                @endif
                            </td>
                            <td class="text-end px-4">
                                ...
                                @else
                                    <span class="text-muted small">Traité <i class="fas fa-check-double ms-1"></i></span>
                                @endif
                            </td>
                        </tr>
                        @empty
